...
public final class Main {
    ...
    private static Map<String, TransactionDAG> buildTransactionDAGs(String[] transactionList){
        ...
        Map<String, String> globalProducerMap = new HashMap<>();

        if(transactions.contains(UPDATE_PRICE)) {
            TransactionBootstrap boot = TransactionBootstrap.name(UPDATE_PRICE)
                    .input("a", "product", UPDATE_PRICE)
                    .terminal("b", "cart", PRICE_UPDATED, "a");
            globalProducerMap.putAll(boot.getInputEventToProducerVMSMap());
            TransactionDAG updatePriceDag = boot.build();
            transactionMap.put(updatePriceDag.name, updatePriceDag);
        }

        if(transactions.contains(UPDATE_PRODUCT)) {
            TransactionBootstrap boot = TransactionBootstrap.name(UPDATE_PRODUCT)
                    .input("a", "product", UPDATE_PRODUCT)
                    .terminal("b", "stock",PRODUCT_UPDATED,"a")
                    .terminal("c", "cart", PRODUCT_UPDATED,"a");
            globalProducerMap.putAll(boot.getInputEventToProducerVMSMap());
            TransactionDAG updateProductDag = boot.build();
            transactionMap.put(updateProductDag.name, updateProductDag);
        }

        if(transactions.contains(UPDATE_DELIVERY)) {
            TransactionBootstrap boot = TransactionBootstrap.name(UPDATE_DELIVERY)
                    .input("a", "shipment", UPDATE_DELIVERY)
                    .terminal("b", "order", SHIPMENT_UPDATED, "a");
            globalProducerMap.putAll(boot.getInputEventToProducerVMSMap());
            TransactionDAG updateDeliveryDag = boot.build();
            transactionMap.put(updateDeliveryDag.name, updateDeliveryDag);
        }

        if(transactions.contains(CUSTOMER_CHECKOUT)) {
            TransactionBootstrap boot= TransactionBootstrap.name(CUSTOMER_CHECKOUT)
...
//                    .terminal("b", "cart", "a")
        ...
        try {
            new ObjectMapper()
                    ...
                    .writeValue(new File("/tmp/map.json"), globalProducerMap);

            System.out.println(">>> Sidecar Map ready at /tmp/map.json");
        } catch (Exception e) {
