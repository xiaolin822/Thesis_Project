...
import dk.ku.di.dms.vms.sdk.embed.client.external.*;
import dk.ku.di.dms.vms.modb.api.annotations.ExternalMsg;
...
import dk.ku.di.dms.vms.sdk.embed.client.external.IExternalSystemClient;
...
public final class VmsEventHandler extends ModbHttpServer {
    ...
    private final String ExternalLogPath;
    ...
    public static VmsEventHandler build(// to identify which vms this is
        ...
        try {
            return new VmsEventHandler(me, vmsMetadata,
                    ...
                    httpHandler, serdesProxy,externalClientFactory);
        } catch (IOException e){
    ...
    private final BlockingQueue<ExternalEventRecord> externalEventBuffer;

    private final ExternalClientFactory externalClientFactory;
    ...
    private VmsEventHandler(VmsNode me,
                            ...
                            IHttpHandler httpHandler,
                            IVmsSerdesProxy serdesProxy, ExternalClientFactory externalClientFactory) throws IOException {
        super();
        ...
        this.externalEventBuffer = new LinkedBlockingQueue<>();
        this.externalClientFactory = externalClientFactory;
        this.options = options;
        this.httpHandler = httpHandler;

        String baseDir = System.getenv().getOrDefault("VMS_LOG_BASE_DIR", "/tmp/");
        if (!baseDir.endsWith("/")) baseDir += "/";
        this.ExternalLogPath = baseDir + me.identifier + "_log.txt";
        LOGGER.log(INFO, me.identifier + ": Log path initialized to " + this.ExternalLogPath);
    ...
    private void checkpoint(long batch, long maxTid) {
        ...
        writeBatchCommitLog(batch, maxTid);
        
        LOGGER.log(INFO, this.me.identifier + ": Checkpoint for batch " + batch + " complete.");
    ...
    private void sendBufferedExternalEvents(long committedBatchId) {
        LOGGER.log(INFO, STR."EH: try flush external buffer up to batch=\{committedBatchId}, current-size=\{this.externalEventBuffer.size()}");
        List<ExternalEventRecord> buf = new ArrayList<>();
        externalEventBuffer.drainTo(buf);
        if (buf.isEmpty()) return;

        List<ExternalEventRecord> toSend = new ArrayList<>();
        List<ExternalEventRecord> keep  = new ArrayList<>();

        for (ExternalEventRecord rec : buf) {
            if (rec.batchId() <= committedBatchId) {
                toSend.add(rec);
            } else {
                keep.add(rec);
            }
        ...
        for (ExternalEventRecord rec : keep) {
            externalEventBuffer.offer(rec);
        }

        for (ExternalEventRecord rec : toSend) {
            try {
                IExternalSystemClient client = externalClientFactory.get(rec.system());
                Class<?> clazz = this.vmsMetadata.queueToEventMap().get(rec.event().outputQueue());
                String payload = this.serdesProxy.serialize(rec.event().output(), clazz);

                int n = Math.max(1, rec.count());
                for (int i = 0; i < n; i++) {
                    client.send(rec.topic(), payload)
                            .exceptionally(ex -> {
                                LOGGER.log(System.Logger.Level.ERROR,
                                        "external send failed: alias=" + rec.system()
                                                + ", topic=" + rec.topic()
                                                + ", err=" + ex.getMessage());
                                // TODO: retry / DLQ
                                return null;
                ...
                LOGGER.log(System.Logger.Level.ERROR,
                        "external dispatch error: alias=" + rec.system()
                                + ", topic=" + rec.topic() + ", err=" + e.getMessage());
                // TODO: retry / DLQ
            }
        }
    }
    ...
    private void writeRawAuditLog(long batch, long tid, String msgType, boolean emit,
        ...
        try {
            ...
            String logLine = String.format("%d|%s|%d|%s|%b|%s|%s|%s|%s|%s%n",
                    batch,
                    this.me.identifier,
                    tid,
                    msgType,
    ...
    private void writeBatchCommitLog(long batch, long maxTid) {
        writeRawAuditLog(
                batch,
                maxTid,
                "COMMIT",
                ...
                "[]",   // RS
                "[]",   // WS
                "BATCH_FINISHED" // Payload
        );
        LOGGER.log(INFO, "EH: COMMIT for Batch " + batch + " written to log.");
    }
    private void writeAuditLog(OutboundEventResult outputEvent) {
        try {
            String rs = (outputEvent.readSet() == null || outputEvent.readSet().isEmpty())
                    ? "[]" : "[" + String.join(",", outputEvent.readSet()) + "]";
            String ws = (outputEvent.writeSet() == null || outputEvent.writeSet().isEmpty())
                    ? "[]" : "[" + String.join(",", outputEvent.writeSet()) + "]";

            String payloadJson = "VOID_RESULT";
            if (outputEvent.output() != null) {
                Class<?> clazz = this.vmsMetadata.queueToEventMap().get(outputEvent.outputQueue());
                payloadJson = this.serdesProxy.serialize(outputEvent.output(), clazz);
            }

            VmsTransactionSignature signature = outputEvent.signature();
            String externalTopic = signature.externalTopic();
            boolean emit = signature.emit();
            long tid = outputEvent.tid();
            String inputmsg = outputEvent.inputQueue();
            ...
            LOGGER.log(ERROR, "Failed to write audit log: " + e.getMessage());
        }
        LOGGER.log(INFO, "EH: Event written to TEXT LOG -> " + this.ExternalLogPath);
    }
    /**
    ...
    private void processOutputEvent(OutboundEventResult outputEvent, String precedenceMap){
        ...
        if (signature == null) {
            LOGGER.log(INFO, "EH: producer signature NOT FOUND for outputQueue="+outputEvent.outputQueue());
        } else {
            LOGGER.log(INFO, "EH: outputQueue="+outputEvent.outputQueue()
                    + ", producer="+ signature.method().getDeclaringClass().getSimpleName()
    ...
    private InboundEvent buildInboundEvent(TransactionEvent.Payload payload){
        ...
        String inputQueue = payload.event();
        ...
        return new InboundEvent( payload.tid(), precedenceMap.get(this.me.identifier),
                payload.batch(), payload.event(), clazz, input, inputQueue);
    }
    ...
    public void close() {
        ...
        try {
            if (this.externalClientFactory != null) {
                this.externalClientFactory.closeAll();
            }
