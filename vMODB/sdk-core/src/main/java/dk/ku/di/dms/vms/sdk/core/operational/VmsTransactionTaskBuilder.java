...
public final class VmsTransactionTaskBuilder {
    ...
    public final class VmsTransactionTask implements Runnable {
        ...
        private void readOnlyRun(){
            try{
                ...
                TransactionContext itxCtx = (TransactionContext) txCtx;
                OutboundEventResult eventOutput = new OutboundEventResult(this.tid, this.batch, this.signature.outputQueue(), output,
                        new java.util.HashSet<>(itxCtx.readSet),
                        new java.util.HashSet<>(itxCtx.writeSet) ,
                        this.inputQueue, this.signature);
        ...
        public void run() {
            ...
            try {
                Object output = this.signature.method().invoke(this.signature.vmsInstance(), this.inputEvent);
                TransactionContext itxCtx = (TransactionContext) txCtx;
                OutboundEventResult eventOutput = new OutboundEventResult(this.tid, this.batch, this.signature.outputQueue(), output, new java.util.HashSet<>(itxCtx.readSet),
                        new java.util.HashSet<>(itxCtx.writeSet), this.inputQueue,this.signature);
    ...
    public VmsTransactionTask build(long tid, long lastTid, long batch,
                                    ...
                                    Object input,
                                    String inputQueue){
        return new VmsTransactionTask(tid, lastTid, batch, signature, input, inputQueue);
    }

    public VmsTransactionTask buildFinished(long tid){
        var sig = new VmsTransactionSignature(null, null, null, ExecutionModeEnum.SINGLE_THREADED, null, null, null, false);
        var deadTask = new VmsTransactionTask(tid, 0, 0, sig, null, "SYSTEM");
