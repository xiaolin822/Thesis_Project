...
public final class VmsMetadataLoader {
    ...
    public static VmsRuntimeMetadata load(Reflections reflections,
        ...
        Map<String, VmsTransactionSignature> outputQueueToProducerSignature = new HashMap<>();
        ...
        mapVmsMethodInputOutput(transactionalMethods,
                ...
                inputOutputEventDistinction, queueToVmsTransactionMap, outputQueueToProducerSignature);
        ...
        // event to producer vms
        Map<String, String> eventToProducerMap = new HashMap<>();

        for (var entry : outputQueueToProducerSignature.entrySet()) {
            String eventName = entry.getKey();
            VmsTransactionSignature sig = entry.getValue();

            Class<?> vmsClass = sig.method().getDeclaringClass();
            String vmsName = clazzNameToVmsNameMap.get(vmsClass.getCanonicalName());
            System.out.println("DEBUG: Registering producer: Event [" + eventName + "] -> Producer [" + vmsName + "]");
            if (vmsName != null) {
                eventToProducerMap.put(eventName, vmsName);
            }
        }
        ...
        return new VmsRuntimeMetadata(
                ...
                outputQueueToProducerSignature,
                eventToProducerMap);
    }
    ...
    private static void mapVmsMethodInputOutput(Set<Method> transactionalMethods,
                                                ...
                                                Map<String, VmsTransactionSignature> outputQueueToProducerSignature) {
        for (Method method : transactionalMethods) {
            ...
            Optional<Annotation> optionalExternalMsg = Arrays.stream(annotations)
                    .filter(p -> p.annotationType() == ExternalMsg.class).findFirst();
            String externalTopic;
            Boolean emit;
            if (optionalExternalMsg.isEmpty()){
                emit = false;
                externalTopic = "default_topic";
            }else{
                externalTopic = ((ExternalMsg) optionalExternalMsg.get()).value();
                emit = ((ExternalMsg) optionalExternalMsg.get()).emit();
            }
            ...
            if(optionalPartitionBy.isPresent()) {
                ...
                try {
                    ...
                    vmsTransactionSignature = new VmsTransactionSignature(obj, method, transactionType, executionMode, Optional.of(partitionMethod), inputQueues, outputQueue, externalTopic, emit);
                } catch (NoSuchMethodException e) {
                    // leave as single threaded
                    vmsTransactionSignature = new VmsTransactionSignature(obj, method, transactionType, executionMode, inputQueues, outputQueue, externalTopic, emit);
                }
            } else {
            ...
            if (outputQueue != null) {
                VmsTransactionSignature prev =
                        outputQueueToProducerSignature.putIfAbsent(outputQueue, vmsTransactionSignature);
                if (prev != null && prev != vmsTransactionSignature) {
                    throw new QueueMappingException(
                            "Error mapping: Output queue '" + outputQueue + "' already produced by another transaction method: "
                                    + prev.method().getDeclaringClass().getSimpleName() + "." + prev.method().getName()
                                    + " vs " + vmsTransactionSignature.method().getDeclaringClass().getSimpleName()
                                    + "." + vmsTransactionSignature.method().getName()
                    );
                }
            }
